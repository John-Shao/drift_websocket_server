...
def test_start_rtsp_api():
    ...
    payload = {
        ...
        "event": "start_rtsp",
        "data": {
            "rtsp_url": "rtsp://14.103.74.174/live/audio",
            "audiodec_samplerate": 16000
        }
    }
    
    print(f"测试START_RTSP接口:")
    print(f"请求URL: {url}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0:
                ...
                print(f"\nSTART_RTSP命令执行失败: {result.get('error_msg', '未知错误')}")
                return False
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("code") == 0:
                ...
                print(f"\nSTOP_RTSP命令执行失败: {result.get('error_msg', '未知错误')}")
                return False
        else:
            print(f"\n请求失败: HTTP {response.status_code}")
            return False
...
def main():
    ...
    # 测试START_RTSP
    start_success = test_start_rtsp_api()

    # 等待指定秒数后测试STOP_RTSP
    if start_success:
        sec = 60
        print(f"\n等待{sec}秒后测试STOP_RTSP...")
        time.sleep(sec)
