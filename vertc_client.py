...
class VertcClient:
    def __init__(self):
        ...
        self.rtc_service.set_ak(settings.volc_access_key)
        self.rtc_service.set_sk(settings.volc_secret_key)
...
if __name__ == "__main__":
    ...
    if test_mode == TestMode.AUDIO_STOP:
        # 测试停止合流转推
        response = client.stop_push_stream_to_cdn(
            room_id="100",
            task_id="200"
        )
        print(response)
        ...
        # 测试停止在线媒体流输入
        response = client.stop_relay_stream(
            room_id="100",
            task_id="100"
        )
        print(response)
        ...
        # 测试关闭实时对话式AI
        response = client.stop_voice_chat(
            room_id="100",
            task_id="300"
        )
        print(response)
    ...
    if test_mode == TestMode.VIDEO_STOP:
        # 测试关闭音视频互动智能体
        response = client.stop_video_chat(
            room_id="100",
            task_id="400"
        )
        print(response)
